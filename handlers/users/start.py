...
async def slp_price(message: Message):
    await message.answer(f'Текущий курс SLP: {slp_take_price(get_user_info(message.from_user.id)[0][4])} {get_user_info(message.from_user.id)[0][4]}')

@dp.message_handler(Text(equals=axs_price.text))
async def axs_price(message: Message):
    await message.answer(f'Текущий курс AXS: {axs_take_price(get_user_info(message.from_user.id)[0][4])} {get_user_info(message.from_user.id)[0][4]}')
